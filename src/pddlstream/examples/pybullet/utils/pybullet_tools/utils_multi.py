...
from motion_planners.rrt import rrt, rrt_sub, rrt_dual
...
# def get_extend_fn_dual(body1, joints1, body2, joints2, resolutions=None, norm=2):
#     # norm = 1, 2, INF
#     resolutions = get_default_resolutions(body1, joints1, resolutions)
#     difference_fn1 = get_difference_fn(body1, joints1)
#     difference_fn2 = get_difference_fn(body2, joints2)
#     def fn(r1q1, r1q2, r2q1, r2q2):
#         #steps = int(np.max(np.abs(np.divide(difference_fn(q2, q1), resolutions))))
#         steps1 = int(np.linalg.norm(np.divide(difference_fn1(r1q2, r1q1), resolutions), ord=norm))
#         steps2 = int(np.linalg.norm(np.divide(difference_fn2(r2q2, r2q1), resolutions), ord=norm))
#         refine_fn1 = get_refine_fn(body1, joints1, num_steps=steps1)
#         refine_fn2 = get_refine_fn(body2, joints2, num_steps=steps2)
#         return {'robot1':refine_fn1(r1q1, r1q2),'robot2':refine_fn2(r2q1,r2q2)}
#     return fn
...
def get_collision_fn_sub(body1, body2, joints1, joints2, obstacles, attachments, self_collisions, disabled_collisions,
                     custom_limits={}, use_aabb=False, cache=False, max_distance=MAX_DISTANCE, **kwargs):
    # TODO: convert most of these to keyword arguments
    # check_link_pairs = get_self_link_pairs(body1, joints, disabled_collisions) if self_collisions else []
    moving_links1 = frozenset(get_moving_links(body1, joints1))
    moving_links2 = frozenset(get_moving_links(body2, joints2))
    attached_bodies = [attachment.child for attachment in attachments]
    
    # TODO: sort bodies by bounding box size
    check_link_pairs = []
    for robot1_link in moving_links1:
        for robot2_link in moving_links2:
            collision_pair = (robot1_link, robot2_link)
            check_link_pairs.append(collision_pair)

    def collision_fn_sub(q1, q2, verbose=True):

        set_joint_positions(body1, joints1, q1)
        set_joint_positions(body2, joints2, q2)
        # time.sleep(0.3)
        for attachment in attachments:
            attachment.assign()
        #wait_for_duration(1e-2)
...
# def plan_joint_motion_dual_arm(body1, body2, joints1, joints2, end_conf1, end_conf2, goal_ee1, goal_ee2,
#                                attachments1=[], attachments2=[], # body: the robot
#                                obstacles=[], self_collisions=True, disabled_collisions=set(),
#                                weights=None, resolutions=None, max_distance=MAX_DISTANCE,
#                                use_aabb=False, cache=True, custom_limits={}, **kwargs):
...
#     # Planning Robot 1
#     # -----------------
#     assert len(joints1) == len(end_conf1)
#     if (weights is None) and (resolutions is not None):
#         weights = np.reciprocal(resolutions)
#     sample_fn = get_sample_fn(body1, joints1, custom_limits=custom_limits)
#     distance_fn = get_distance_fn(body1, joints1, weights=weights)
#     extend_fn = get_extend_fn(body1, joints1, resolutions=resolutions)
#     collision_fn = get_collision_fn(body1, joints1, obstacles, attachments1, self_collisions, disabled_collisions,
#                                     custom_limits=custom_limits, max_distance=max_distance,
#                                     use_aabb=use_aabb, cache=cache)


#     start_conf1 = get_joint_positions(body1, joints1)

#     if not check_initial_end(start_conf1, end_conf1, collision_fn):
#         robot1_path = None
#     robot1_path = rrt(start_conf1, end_conf1, distance_fn, sample_fn, extend_fn, collision_fn, **kwargs)
#     #return plan_lazy_prm(start_conf, end_conf, sample_fn, extend_fn, collision_fn)

#     # -----------------
#     # Planning Robot 2
#     # -----------------
#     assert len(joints2) == len(end_conf2)
#     if (weights is None) and (resolutions is not None):
#         weights = np.reciprocal(resolutions)
#     sample_fn = get_sample_fn(body2, joints2, custom_limits=custom_limits)
#     distance_fn = get_distance_fn(body2, joints2, weights=weights)
#     extend_fn = get_extend_fn(body2, joints2, resolutions=resolutions)
#     collision_fn = get_collision_fn(body2, joints2, obstacles, attachments2, self_collisions, disabled_collisions,
#                                     custom_limits=custom_limits, max_distance=max_distance,
#                                     use_aabb=use_aabb, cache=cache)

#     collision_fn_sub = get_collision_fn_sub(body1, body2, joints1, joints2, obstacles, attachments2, self_collisions, disabled_collisions,
#                                     custom_limits=custom_limits, max_distance=max_distance,
#                                     use_aabb=use_aabb, cache=cache)

#     start_conf2 = get_joint_positions(body2, joints2)

#     if not check_initial_end(start_conf2, end_conf2, collision_fn):
#         robot2_path = None
#     robot2_path = rrt_sub(start_conf2, end_conf2, distance_fn, sample_fn, extend_fn, collision_fn, collision_fn_sub, robot1_path, **kwargs)

#     return {'robot1': robot1_path, 'robot2': robot2_path}




def plan_joint_motion_dual_arm(body1, body2, joints1, joints2, end_conf1, end_conf2, end_ee1, end_ee2,
                               attachments1=[], attachments2=[], # body: the robot
                               obstacles=[], self_collisions=True, disabled_collisions=set(),
                               weights=None, resolutions=None, max_distance=MAX_DISTANCE,
                               use_aabb=False, cache=True, custom_limits={}, **kwargs):
...
#(start1, goal_sample1, start2, goal_sample2, distance, sample, extend, collision, collision_sub, goal_test=lambda q: False, iterations=1000, goal_probability=.1):
    path = rrt_dual(start_conf1, end_conf1, end_ee1, start_conf2, end_conf2, end_ee2, distance_fn, sample_fn, extend_fn, collision_fn, collision_fn_sub)
    return path
