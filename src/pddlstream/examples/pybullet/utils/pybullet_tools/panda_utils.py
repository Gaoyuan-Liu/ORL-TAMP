...
GRASP_LENGTH = 0.01 # Depth of the gripper fingers
...
SIDE_EDGE_OFFSET = 0.00 # x distance from edge of object
HAND_LENGTH = 0.11# 0.11
...
def get_side_grasps(body, under=False, tool_pose=TOOL_POSE, body_pose=unit_pose(),
    ...
    grasps = []
    #x_offset = 0
    x_offset = h/2  #top_offset # This is the z distance from the top of the object, h/2 means the top edge
    for j in range(1 + under):
        fixed_y_rotate = Pose(euler=[0, math.pi / 2 + j * math.pi, 0])

        n_bbq_rotations = 8
        if w <= max_width:
            translate_z = Pose(point=[x_offset, 0, l / 2 - grasp_length + HAND_LENGTH])

            for i in range(n_bbq_rotations):
                # rotate_z = Pose(euler=[math.pi / 2 + i * math.pi, 0, 0])
                rotate_z = Pose(euler=[i * 2 * math.pi / n_bbq_rotations, 0, 0])
                grasps += [multiply(tool_pose, translate_z, rotate_z, fixed_y_rotate,
                                    translate_center, body_pose)]  # , np.array([w])
        if l <= max_width:
            translate_z = Pose(point=[x_offset, 0, w / 2 - grasp_length + HAND_LENGTH])
            for i in range(n_bbq_rotations):
...
def get_edge_grasps(body, body_type='cylinder', tool_pose=TOOL_POSE, body_pose=unit_pose(),
                    max_width=MAX_GRASP_WIDTH, grasp_length=GRASP_LENGTH, top_offset=SIDE_EDGE_OFFSET):
    # TODO: compute bounding box width wrt tool frame
    center, (w, l, h) = approximate_as_prism(body, body_pose=body_pose)
    translate_center = Pose(point=point_from_pose(body_pose)-center) # ((0,0,0),(0,0,0,1)))
    # print(f'\n center = {translate_center} \n')
    grasps = []
    # if body_type == 'cylinder':
    #     fixed_x_rotate = Pose(euler=[math.pi / 2, 0, 0])
    #     how_many_cuts_for_the_pie = 8
    #     for i in range(how_many_cuts_for_the_pie):
    #         angle = i * 2 * math.pi/how_many_cuts_for_the_pie
    #         fixed_y_rotate = Pose(euler=[0, angle, 0 ])
    #         distance_to_center = w/2
    #         translate_z = Pose(point=[0, 0, distance_to_center - grasp_length + HAND_LENGTH])            
    #         grasps += [multiply(tool_pose, translate_z, fixed_y_rotate,  fixed_x_rotate,
    ...
    if body_type == 1:
        ...
        how_many_cuts_for_the_pie = 8
        for i in range(how_many_cuts_for_the_pie):
            # rotate_z = Pose(euler=[i * math.pi/4, 0, 0 ])
            angle = i * 2 * math.pi/how_many_cuts_for_the_pie
            fixed_y_rotate = Pose(euler=[0, angle, 0 ])

            distance_to_center = min([abs(l/np.cos(angle)), abs(w/np.sin(angle))])/2
            translate_z = Pose(point=[0, 0, distance_to_center - grasp_length + HAND_LENGTH])

            # grasps += [multiply(tool_pose, translate_z, swap_xy, rotate_z, swap_xz,
            #                     translate_center, body_pose)]  # body_pose is empty here, grasp is T_{o2ee}
    ...
    # else:
    #     print('Current support body_type = cylinder or box.' )
