...
from pushing_streams import get_loose_goal_push_gen, get_push_initial_test, get_same_surface_test, get_pushing_gen
from hooking_streams import get_loose_goal_hook_gen, get_hooking_gen
...
class Solver():
    def __init__(self, robot, movable=[], tools=[], surfaces=[0], obstacles=[], marks=[], edge_grasps=[], top_grasps=[]):
        ...
        self.grasp_types = ('edge', 'top')
        self.movable = movable
        self.tools = tools
        self.marks = marks
        self.pusher = Pushing()

        self.edge_grasps = edge_grasps
        self.top_grasps = top_grasps
    ...
    def problem_formation(self):
        ...
        self.problem.edge_grasps = self.edge_grasps
        self.problem.top_grasps = self.top_grasps
    ...
    def pddlstream_from_problem(self, collisions=True, teleport=False):
        ...
        stream_map = {
            ...
            'sample-loose-goal-push': from_gen_fn(get_loose_goal_push_gen(self.problem, collisions=collisions)),
            'sample-pushing': from_gen_fn(get_pushing_gen(self.problem, self.pusher, collisions=collisions)),
            'sample-loose-goal-hook': from_gen_fn(get_loose_goal_hook_gen(self.problem, collisions=collisions)),
            'sample-hooking': from_gen_fn(get_hooking_gen(self.problem, collisions=collisions)),
            ...
            'test-push-initial-pose': from_test(get_push_initial_test(self.problem, collisions=collisions)),
            'test-same-surface': from_test(get_same_surface_test(self.problem, collisions=collisions)),
    ...
    # def execution(self, solution):
    #     plan, cost, evaluations = solution

    #     # print(f'\n plan = {plan}')
    #     # print(f'\n type(plan[0]) = {plan[0][0]}')
    ...
    #         commands = post_process(self.problem, plan, teleport=False)
    ...
    # def solve_and_execute(self, goal):
    #     solution = self.solve(goal)
    #     plan, _, _ = solution
        
    #     if plan!=None:
    #         solved = True
    #         self.execution(solution)
...
if __name__ == '__main__':
    ...
    table1 = scenario.add_table(pose=((0.5,0,0), (0,0,0,1)))
    table2 = scenario.add_table(pose=((-0.5,0,0), (0,0,0,1)))
    scenario.add_dish(pose=((0.5, 0.24, 0.31),(0,0,0,1)))
    # scenario.add_cube(pose=((0.5, 0.0, 0.31),(0,0,0,1)))

    p.setRealTimeSimulation(1)
    time.sleep(1)
    p.setRealTimeSimulation(0)

    # print(f'\n scenario.dish = {scenario.dish}')
    print(f'\n table1 = {table1}')
    print(f'\n table2 = {table2}')

    # ans = p.getCollisionShapeData(scenario.dish, -1)
    # print(f'\n ans = {ans}\n')

    # ans = p.getCollisionShapeData(scenario.cube, -1)
    # print(f'\n ans = {ans}\n')
    ...
    # execute(solver.problem, plan, scenario)
