...
def get_loose_goal_push_gen(problem, collisions=True, **kwargs):
    data = pd.read_csv('./../state_discriminator_push/data/data_push.csv')
    ...
    def gen(body):
        ...
        body_pose = ((0.4, 0.25, 0.325), (0.0, 0.0, 0.0, 1.0))
...
def get_loose_goal_hook_gen(problem, collisions=True, **kwargs):
    # goals = load_list('./../state_discriminator/goals')
    def gen(body):
        while True:
            # goal = random.choice(goals)
            # body_pose = ((goal[0], goal[1], 0.036), (0.0, 0.0, 0.0, 1.0))
            body_pose = ((0.5, 0.1, 0.336), (0.0, 0.0, 0.0, 1.0))
            ap = Pose(body, body_pose)
            yield (ap,)
    return gen
