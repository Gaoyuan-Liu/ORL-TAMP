...
class EdgePushEnv(gym.Env):
    ...
    def connect(self):
        ...
        self.robot = self.scn.add_robot(pose=((0,0,0), (0,0,0,1)))
        
        # Table
        self.table = self.scn.add_low_table() # The height of the table surface is 0.3

        # Objects
        # self.target, self.body_type = self.scn.add_dish_random()
        self.target = self.scn.add_dish()
        self.body_type = 0
        ...
        self.obj_info = {self.robot: 'robot', self.table: 'table', self.target: 'dish'}

        self.obstacles = [self.table, self.target]
    ...
    def reset(self):
        ...
        self.target, self.body_type = self.scn.add_dish_random() # 0 cylinder, 1 box
    ...
    def step(self, action):
        ...
        self.render(action)
        # Reset Robot
        # self.scn.reset_robot(self.robot)

        # state = pb.getBasePositionAndOrientation(self.target)
    ...
    def render(self, action):
        ...
        # ----------------------------------------------
        self.pushing.push(self.robot, start_point, end_point, self.obstacles)
