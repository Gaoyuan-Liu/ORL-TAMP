...
class PandaSim(object):
  def __init__(self, bullet_client, offset):
    ...
    # Add my objects
    # print(f'client in grasp = {self.bullet_client}')
    my_scenario = Scenario(bullet_client=self.bullet_client)
    my_scenario.add_hook(pose=((0,0.1,-0.4),(0,0,0,1)))
    self.legos[0] = my_scenario.hook
    pose,o = self.bullet_client.getBasePositionAndOrientation(my_scenario.hook)
    print(pose)
  ...
  def step(self):
    ...
    #
    # state 4 is going down
    # state 7 is in hand moving


    if self.state==6: # close finger
      self.finger_target = 0.01
    if self.state==5: # release finger
      self.finger_target = 0.04 
    # self.bullet_client.submitProfileTiming("step")

    self.update_state()
    # print(f'Now state = {self.state}')
    ...
    if self.state==1 or self.state==2 or self.state==3 or self.state==4 or self.state==7:
      ...
      if self.state == 2 or self.state == 3 or self.state == 7:
        self.gripper_height = alpha * self.gripper_height + (1.-alpha)*0.2 # above object
...
#   def update_state(self): # This function replaced the original one
