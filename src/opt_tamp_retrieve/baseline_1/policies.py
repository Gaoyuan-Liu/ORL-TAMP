...
class HookPolicyIdeal():
    def __init__(self, scn, focused_object, pose, goal, planner):
        ...
        self.pose = pose
        self.goal = goal
        self.objects = [self.focused_object, self.scn.hook]
    ...
    def apply(self, _):
        ...
        # p.setRealTimeSimulation(0)
        pos, orn = p.getBasePositionAndOrientation(self.focused_object)
        ...
        y_0 = pos[1] - 0.1
        pos_0 = [x_0, y_0, 0.02]
        orn_0 = quaternion_from_euler(0,0,0)
        # p.resetBasePositionAndOrientation(self.objects[-1], pos_0, orn_0)
        ...
        p.setRealTimeSimulation(0)
        p.resetBasePositionAndOrientation(self.scn.hook, pos_0, orn_0)
        # plan, cost, evaluations = self.solver.solve(goal_0, collisions=False)
        ...
        # d_theta = action[3] * math.pi/2
        # d_r = 0.55 + action[4] * 0.15
        d_yaw = 0#action[5] * math.pi/2

        # d_x = d_r * math.cos(d_theta)
        # d_y = d_r * math.sin(d_theta)
        d_x = self.goal.value[0][0] - 0.4
        d_y = self.goal.value[0][1] - 0.1
