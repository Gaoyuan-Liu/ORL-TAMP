...
def get_cfree_approach_pose_test(problem, collisions=True):
    ...
    p.resetJointState(gripper, 1, targetValue=0.04, targetVelocity=0)
    p.resetJointState(gripper, 2, targetValue=0.04, targetVelocity=0)
...
def get_far_pose_test(problem, collisions=True):
    def test(b, p):
        p.assign()
        # if pairwise_collision(b1, b2):
        #     return False
        if get_distance(p.value[0][:2], [0,0]) < 0.7:
            return False
        return True
    return test


def get_near_pose_test(problem, collisions=True):
    def test(b, p):
        # print('\n hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh\n')
        p.assign()
        # if pairwise_collision(b1, b2):
        #     return False
        # dis = get_distance(p.value[0][:2], [0,0])
        # print(f'\n dis: {dis}')
        if get_distance(p.value[0][:2], [0,0]) > 0.7:
            return False
        return True
