...
ax = filtered_loss_center.reset_index().plot(x='index', y='loss_center', kind='line',
                        ...
                        fontsize=fontsize,
                        xticks=np.arange(0, (len(filtered_loss_center)//100)*100 + 400, step=500),
                        color=color_1)
...
plt.savefig("loss_retrieve.pdf", bbox_inches="tight")
plt.show()
...
ax = normalized_accu_center.reset_index().plot(x='index', y='accu_center', kind='line',
                        linewidth=linewidth_1,
                        fontsize=fontsize,
                        xticks=np.arange(0, (len(normalized_accu_center)//100)*100 + 100, step=400),
                        color=color_1)
...
plt.grid(color = 'lightgrey', linestyle = '--', linewidth = 1)
plt.savefig("accu_retrieve.pdf", bbox_inches="tight")
plt.show()
