...
# Episode Return
# ---------------
return_center = normalize_dataframe(return_center)
return_up = normalize_dataframe(return_up)
return_low = normalize_dataframe(return_low)

return_center = moving_average_filter(return_center, window)
return_up = moving_average_filter(return_up, window)
return_low = moving_average_filter(return_low, window)


ax = return_center.reset_index().plot(x='index', y='return_center', kind='line',
                        linewidth=linewidth_1,
                        fontsize=fontsize,
                        xticks=np.arange(0, (len(return_center)//100)*100 + 100, step=400),
                        color=color_1)

plt.fill_between(return_center.index, return_up, return_low, alpha=0.2, color=color_1, edgecolor='none')
...
plt.savefig("er_retrieve.pdf", bbox_inches="tight")
plt.show()
