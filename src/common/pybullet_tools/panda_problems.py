...
# For panda    
from utils.pybullet_tools.utils import add_data_path, connect, dump_body, disconnect, wait_for_user, \
...
from utils.pybullet_tools.ikfast.franka_panda.ik import PANDA_INFO, FRANKA_URDF, SAFE_FRANKA_URDF
from utils.pybullet_tools.ikfast.ikfast import get_ik_joints, either_inverse_kinematics, check_ik_solver

LIGHT_GREY = (0.7, 0.7, 0.7, 1.)

class Problem(object):
    def __init__(self, robot, robots=tuple(), obj_poses={}, robot_positions={}, obstacles=tuple(), movable=tuple(), grasp_types=tuple(),
        ...
        all_movable = list(self.robots) + list([self.robot]) + list(self.movable)#[self.robot] + list(self.movable)
        self.fixed = list(filter(lambda b: b not in all_movable, get_bodies()))
        self.gripper = None #create_gripper(self.robot, visual=True)
