...
GRASP_LENGTH = 0.02
APPROACH_DISTANCE = 0.04 + GRASP_LENGTH
...
class Grasp(object):
    ...
    def get_attachment(self, arm, arm_): 
        ...
        return Attachment(arm, tool_link, self.value, self.body)
    def __repr__(self):
...
# def get_stable_gen_backup(problem, collisions=True, **kwargs):
...
# def get_place_stable_gen(problem, collisions=True, **kwargs):
#     obstacles = problem.fixed if collisions else []
#     def gen(arm, body, surface):
#         # TODO: surface poses are being sampled in pr2_belief
#         print(f'\n surface = {surface}\n')
#         if surface is None:
#             if arm == problem.robots[0]:
#                 surfaces = [problem.surfaces[0]]
#             else:
#                 surfaces = [problem.surfaces[1]]
#         else:
#             surfaces = [surface]
...
#             # print(f'\n robot_positions = {problem.robot_positions[arm]}\n')
#             # print(f'\n pose_distance = {np.linalg.norm(np.array(body_pose[0])-np.array(problem.robot_positions[arm]))}\n')
#             if np.linalg.norm(np.array(body_pose[0])-np.array(problem.robot_positions[arm]))>0.7: 
#                 break
#             p = Pose(body, body_pose, surface)
#             p.assign()
#             if not any(pairwise_collision(body, obst) for obst in obstacles if obst not in {body, surface}):
...
def get_airpose_gen(problem, collisions=True, **kwargs):
    obstacles = problem.fixed if collisions else []
    def gen(body):
        # TODO: surface poses are being sampled in pr2_belief
        while True:
            body_pose = ((0.0, 0.5, 0.4), (0.0, 0.0, 0.0, 1.0))#sample_placement(body, surface, **kwargs)
            if body_pose is None:
                break
            ap = Pose(body, body_pose)
            # ap.assign() # Fix the object pose
            # print(f'\n stuck here! {body}\n')
            # for obst in obstacles:
            #     if obst not in {body}:
            #         print(obst)
            #         print(pairwise_collision(body, obst))

            if not any(pairwise_collision(body, obst) for obst in obstacles if obst not in {body}):
                yield (ap,)
    # TODO: apply the acceleration technique here
    return gen
...
def get_ik_fn(problem, custom_limits={}, collisions=True, teleport=False):
    ...
    def fn(arm, obj, pose, grasp):
        ...
        # This command will change the robot pose
        ...
        # Approach Pose Collision Checking
