...
def get_simple_ik_fn(robot, fixed=[], teleport=False, num_attempts=10):
    movable_joints = get_movable_joints(robot)
    sample_fn = get_sample_fn(robot, movable_joints)
    tool_link = get_tool_link(robot)
    def fn(pose):
...
def get_cutting_ee_pose():
    # tool_link = get_tool_link(robot)
    def gen(p_c, normal, r):
        # Cutting disk pose should contains the following information:
        # 1. The position of the center of the disk
        # 2. The normal of the disk (quaternion)
        # The output ee pose is defined in the world frame
        # normal = np.array([0, 0, 0])
        v_1 = np.array([normal[1], -normal[0], 0])
        v_2 = np.cross(normal, v_1)
        ...
        for i in range(0, 360, 30):
            p_ee = p_c + r * (np.cos(i * np.pi / 180) * v_1 + np.sin(i * np.pi / 180) * v_2)

            p_ee2p_c = -(p_ee - p_c)
            p_ee2p_c = p_ee2p_c / np.linalg.norm(p_ee2p_c)
            p_ee2p_cxnormal = np.cross(p_ee2p_c, normal)
            p_ee2p_cxnormal = p_ee2p_cxnormal / np.linalg.norm(p_ee2p_cxnormal)

            a = np.array([world_z, world_y])
            b = np.array([p_ee2p_c, p_ee2p_cxnormal])

            ans = Rotation.align_vectors(b, a)
            quat = ans[0].as_quat()


            yield (p_ee, quat)
            # q_ee = quat_from_matrix(np.array([v_1, v_2, normal]).T)
