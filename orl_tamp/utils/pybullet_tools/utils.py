...
def get_num_joints(body):
    # print(f'\nnumjoints = {p.getNumJoints(body, physicsClientId=CLIENT)}')
...
def body_collision(body1, body2, **kwargs):
    ...
    if len(get_closest_points(body1, body2, **kwargs)) != 0:
        print(f"\nbody_collision: {body1}, {body2}")
...
def get_collision_fn(body, joints, obstacles, attachments, self_collisions, disabled_collisions,
    ...
    def collision_fn(q, verbose=True):
        ...
        # Collision with attachment
        for attachment in attachments:
            attachment.child_initial_pose.assign()
        for body1, body2 in product([body], obstacles): 
            # if body1[0] == body2: # Same object
            #     return False
            if body1 == 0 or body2 == 0:
                continue
            if pairwise_collision(body1, body2, max_distance=0.01):#**kwargs):
                # print(get_body_name(body1), get_body_name(body2))
                if verbose: print(f" collision with attachment between {body1} and {body2}")
                # if verbose: print(body1, body2)
                return True
...
def sample_placement_on_aabb(top_body, bottom_aabb, top_pose=unit_pose(),
    ...
    for _ in range(max_attempts):
        ...
        # print(f'\ntop_body: {top_body}, center: {center}, extent: {extent}')
        # print(f'\nbottom_aabb: {bottom_aabb}')
...
def sample_placement(top_body, bottom_body, bottom_link=None, **kwargs):
    print(f'\nbottom_body: {bottom_body}, bottom_link: {bottom_link}')
    bottom_aabb = get_aabb(bottom_body, link=bottom_link)
    return sample_placement_on_aabb(top_body, bottom_aabb, **kwargs)


def sample_edge_on_aabb(top_body, bottom_aabb, top_pose=unit_pose(),
    ...
    for _ in range(max_attempts):
        ...
        # print(f'bottom_aabb[0][0] = {bottom_aabb[0][0]}')
        # print(f'type = {type(bottom_aabb[0][0])}')
        # input()


        x_ranges = [(bottom_aabb[0][0], bottom_aabb[0][0] + percent*extent[0]/2),
                     (bottom_aabb[1][0] - percent*extent[0]/2, bottom_aabb[1][0])]
        
        y_ranges = [(bottom_aabb[0][1], bottom_aabb[0][1] + percent*extent[1]/2),
                     (bottom_aabb[1][1] - percent*extent[1]/2, bottom_aabb[1][1])]
        
        xy = np.random.choice([0,1])
        if xy == 0:
            i = np.random.choice([0,1])
            x = np.random.uniform(x_ranges[i][0], x_ranges[i][1])
            y = np.random.uniform(bottom_aabb[0][1], bottom_aabb[1][1])
        else:
            x = np.random.uniform(bottom_aabb[0][0], bottom_aabb[1][0])
            i = np.random.choice([0,1])
            y = np.random.uniform(y_ranges[i][0], y_ranges[i][1])
...
def sample_edge(top_body, bottom_body, bottom_link=None, **kwargs):
    # print(f'\nbottom_body: {bottom_body}, bottom_link: {bottom_link}')
    bottom_aabb = get_aabb(bottom_body, link=bottom_link)
    return sample_edge_on_aabb(top_body, bottom_aabb, **kwargs)
...
def joint_controller(body, joints, target, tolerance=1e-2, timeout=INF, **kwargs): # tolerance=1e-3
...
def inverse_kinematics(robot, link, target_pose, max_iterations=200, max_time=INF, custom_limits=FRANKA_CUSTOM_LIMITS, **kwargs):
    start_time = time.time()
