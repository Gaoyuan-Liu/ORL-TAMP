...
from .utils import invert, multiply, get_name, set_pose, get_link_pose, is_placement, get_center_extent, sample_edge, \
...
class Trajectory(Command):
    ...
    def apply(self, state, sample=1):
        ...
        for handle in handles:
            remove_debug(handle)
    def control(self, dt=0.05, **kwargs):
        # TODO: just waypoints
...
class Clean(Command):
    ...
    def apply(self, state, **kwargs):
        ...
        yield
    def control(self, **kwargs):
        pb.addUserDebugText('Cleaned', textPosition=(0, 0, .25), textColorRGB=(0,0,1), #textSize=1,
...
class Cook(Command):
    ...
    def control(self, **kwargs):
        ...
        pb.addUserDebugText('Cooked', textPosition=(0, 0, .5), textColorRGB=(1,0,0), #textSize=1,
                           lifeTime=0, parentObjectUniqueId=self.body)
...
def get_grasp_gen(problem, collisions=False, randomize=False):
    ...
    def fn(body):
        ...
        # print(f'\n problem.edge_grasps = {problem.edge_grasps}\n')
...
def get_stable_gen(problem, collisions=True, **kwargs):
    ...
    def gen(body, surface):
        ...
        while True:
            ...
            if body in problem.edge_grasps:
                body_pose = sample_edge(body, surface)
                if np.linalg.norm(np.array(body_pose[0]) - np.array([0,0,0])) > 0.6:
                    continue
...
def replanner(arm, obj, pose, grasp, obstacles, custom_limits={}, collisions=True):
    ...
    if approach_conf is None:
        print(' No ik solution for approach pose.')
        return None
    if any(pairwise_collision(robot, b) for b in obstacles + [obj]):
