...
def extend_towards(tree, target, distance_fn, extend_fn, collision_fn, swap, tree_frequency):
    last = argmin(lambda n: distance_fn(n.config, target), tree) # this is a heuristics, last is the nearest to goal node in the tree, and that node will be set as the last node
    extend = list(asymmetric_extend(last.config, target, extend_fn, swap))
    safe = list(takewhile(negate(collision_fn), extend)) # safe is the collision-free subset of extend
    # print(np.asarray(extend).shape)
...
def birrt(q1, q2, distance, sample, extend, collision,
    ...
    for attempt in irange(restarts + 1):
        ...
        if path is not None:
            ...
            # print(f'\n len of path = {len(path)} \n')

            smoothed_path = smooth_path(path, extend, collision, distance_fn=distance, iterations=smooth,
                               max_time=max_time - elapsed_time(start_time))
            # print(f'\n len of smoothed path = {len(smoothed_path)} \n')
