...
sys.path.insert(0, file_path + '/../')
from utils.scn import add_ground, add_robot, add_table, add_cylinder, add_bar
...
class RetrieveEnv(gym.Env):
    ...
    def connect(self, use_gui=True):
        ...
        pb.configureDebugVisualizer(rgbBackground=[1, 1, 1])
        ...
        # Robot
        robot = add_robot(pose=((0,0,0.0), (0,0,0,1)))
        self.scn['robot'] = robot
        self.control.finger_open(robot)

        # Table
        table_low = add_table(1, 0.5, 0.15, pose=((0.75, 0, 0.075),(0,0,0,1))) #add_low_table()
        self.scn['low_table'] = table_low

        # Cup
        cup = add_cylinder(0.06, 0.03, pose=((0.75, 0, 0.15+0.015),(0,0,0,1)), color=(0.6,0.6,0.6,1))
        self.scn['cup'] = cup
        self.target = cup
    ...
    def reset_train(self):
        ...
        for i in range(pandaNumDofs):
            pb.setJointMotorControl2(self.scn['robot'], i, pb.POSITION_CONTROL, initialJointPoses[i],force=5 * 240.)
        ...
        pb.resetBasePositionAndOrientation(self.scn['bar'], (x_hook, y_hook, 0.16), orn_hook)
    ...
    def reset_eval(self):
        ...
        orn_hook = quaternion_from_euler(0,0,yaw_hook)
        orn_hook = quaternion_from_euler(0,0,0)
        pb.resetBasePositionAndOrientation(self.scn['bar'], (x_hook, y_hook, 0), orn_hook)
    ...
    def observe(self):
        ...
        pos, orn = pb.getBasePositionAndOrientation(self.scn['bar'])
        rpy = pb.getEulerFromQuaternion(orn)
        total.append(pos[0])
        total.append(pos[1])
        total.append(rpy[-1])
    ...
    def render(self, action):
        ...
        # pb.resetBasePositionAndOrientation(self.scn['bar'], pos_0, orn_0)
        ...
        # Go approach
        info = pb.getBasePositionAndOrientation(self.scn['bar'])
        ee_pos1 = list(info[0])
        ...
        joint_states = pb.getJointStates(self.scn['robot'], list(range(pandaNumDofs)))# Given by Holding
        start_conf = []
        ...
        for waypoint in trajectory:
            ...
            finger_state = pb.getLinkState(self.scn['robot'], 11)[4]
            self.control.go_joint_space(self.scn['robot'], waypoint)
        # self.control.finger_open(self.scn['robot'])
        self.control.finger_close(self.scn['robot'])
        # ------------------------------
    ...
    def reward(self, obs):
        ...
        hook_pose = pb.getBasePositionAndOrientation(self.scn['bar'])
